...
class GameDataRepository:
    ...
    def _update(self, table, pk: dict[str, Any], values: dict[str, Any]) -> bool:
        with self._engine.begin() as conn:
            result = conn.execute(
                update(table).where(*(table.c[k] == v for k, v in pk.items())).values(**values)
            )
            return result.rowcount > 0

    def _delete(self, table, **pk) -> bool:
        with self._engine.begin() as conn:
            result = conn.execute(delete(table).where(*(table.c[k] == v for k, v in pk.items())))
            return result.rowcount > 0
    ...
    def delete_game_data(self, game_id: int) -> None:
        ...
        with self._engine.begin() as conn:
            ...
            conn.execute(delete(c.skill_tree_tags).where(c.skill_tree_tags.c.tree_id.in_(tree_ids)))
            conn.execute(delete(c.skill_categories).where(c.skill_categories.c.game_id == game_id))
    ...
    def create_game(
        self, *, code: str, name: str, generation: int, features: str, **extra
    ) -> dict[str, Any]:
        return self._create(
            t.games, dict(code=code, name=name, generation=generation, features=features, **extra)
        )
    ...
    def list_skills_for_tree(self, tree_id: int) -> list[dict[str, Any]]:
        return self._list(t.skills, t.skills.c.tree_id == tree_id, order_by=t.skills.c.points)
    ...
    def set_armor_skill(self, *, armor_id: int, tree_id: int, points: int) -> dict[str, Any]:
        return self._create(
            t.armor_skills, {"armor_id": armor_id, "tree_id": tree_id, "points": points}
        )
    ...
    def set_decoration_skill(
        self, *, decoration_id: int, tree_id: int, points: int
    ) -> dict[str, Any]:
        return self._create(
            t.decoration_skills,
            {"decoration_id": decoration_id, "tree_id": tree_id, "points": points},
        )
    ...
    def list_decoration_skills_for_decoration(self, decoration_id: int) -> list[dict[str, Any]]:
        return self._list(t.decoration_skills, t.decoration_skills.c.decoration_id == decoration_id)
    ...
    def update_decoration_skill(self, decoration_id: int, tree_id: int, **values) -> bool:
        return self._update(
            t.decoration_skills, {"decoration_id": decoration_id, "tree_id": tree_id}, values
        )
    ...
    def create_charm_type(
        self, *, game_id: int, code: str, max_slots: int, **extra
    ) -> dict[str, Any]:
        return self._create(
            t.charm_types, dict(game_id=game_id, code=code, max_slots=max_slots, **extra)
        )
    ...
    def set_charm_skill_range(
        self, *, charm_type_id: int, tree_id: int, skill_slot: int, min_points: int, max_points: int
    ) -> dict[str, Any]:
        return self._create(
            t.charm_skill_ranges,
            {
                "charm_type_id": charm_type_id,
                "tree_id": tree_id,
    ...
    def get_charm_skill_range(
        self, charm_type_id: int, tree_id: int, skill_slot: int
    ) -> dict[str, Any] | None:
        return self._get(
            t.charm_skill_ranges,
            charm_type_id=charm_type_id,
    ...
    def list_charm_skill_ranges(self, charm_type_id: int) -> list[dict[str, Any]]:
        return self._list(
            t.charm_skill_ranges, t.charm_skill_ranges.c.charm_type_id == charm_type_id
        )

    def update_charm_skill_range(
        self, charm_type_id: int, tree_id: int, skill_slot: int, **values
    ) -> bool:
        return self._update(
            t.charm_skill_ranges,
    ...
    def delete_charm_skill_range(self, charm_type_id: int, tree_id: int, skill_slot: int) -> bool:
        return self._delete(
            t.charm_skill_ranges,
            charm_type_id=charm_type_id,
            tree_id=tree_id,
    ...
    def set_charm_slot_threshold(
        self, *, charm_type_id: int, fulfillment: int, slots: int
    ) -> dict[str, Any]:
        return self._create(
            t.charm_slot_thresholds,
            {"charm_type_id": charm_type_id, "fulfillment": fulfillment, "slots": slots},
        )

    def get_charm_slot_threshold(
        self, charm_type_id: int, fulfillment: int
    ) -> dict[str, Any] | None:
        return self._get(
            t.charm_slot_thresholds, charm_type_id=charm_type_id, fulfillment=fulfillment
        )
    ...
    def update_charm_slot_threshold(self, charm_type_id: int, fulfillment: int, **values) -> bool:
        return self._update(
            t.charm_slot_thresholds,
    ...
    def create_mh3u_charm_table_entry(
        self,
        *,
        game_id: int,
        table_index: int,
    ...
    def list_mh3u_charm_table(
        self, game_id: int, table_index: int | None = None
    ) -> list[dict[str, Any]]:
