...
def test_loaded_row_counts(etl_db):
    ...
    assert table_counts(repo, game_id) == {k: v for k, v in EXPECTED_COUNTS.items() if k != "games"}
...
def test_chain_helm_spot_check_in_db(etl_db):
    ...
    skills = {
        tree_names[s["tree_id"]]: s["points"] for s in repo.list_armor_skills_for_piece(helm["id"])
    }
    assert skills == {"Paralysis": -1, "Health": 2, "Backpackng": 2, "Map": 2, "Whim": 2}
...
def test_negative_skills_flagged(etl_db):
    repo, game_id, _ = etl_db
    attack = next(t for t in repo.list_skill_trees(game_id) if t["name_en"] == "Attack")
...
def test_rebuild_is_idempotent_and_preserves_user_data(etl_db, manifest, pack_data):
    ...
    attack_tree = next(t for t in repo.list_skill_trees(game_id) if t["name_en"] == "Attack")
    charm = user_repo.add_charm(
        session_id=session["id"],
        game_id=game_id,
        slots=2,
        skill1_tree=attack_tree["id"],
        skill1_points=5,
    )
    state = user_repo.create_search_state(
        session_id=session["id"], game_id=game_id, query_json="{}"
    )
...
def test_deterministic_piece_ids_across_rebuilds(etl_db, manifest, pack_data):
    repo, game_id, _ = etl_db
    before = {
        p["name_en"]: p["id"] for p in repo.list_armor_pieces(game_id, slot=0, allow_event=True)
    }
